...
def main():
    ...
    criterion = {'BCELoss': BCELoss(reduce=True, size_average=True).to(device),
                 'IntraBCELoss': BCELoss(margin=args.intraBCEMargin, reduce=True, size_average=True).to(device),
                 'InterBCELoss': BCELoss(margin=args.interBCEMargin, reduce=True, size_average=True).to(device),
                 'IntraCooccurrenceLoss' : intraAsymmetricLoss(args.classNum, gamma_neg=2, gamma_pos=1, reduce=True, size_average=True).to(device),
                 'InterDistanceLoss': ContrastiveLoss(args.batchSize, reduce=True, size_average=True).to(device),
                 }
    ...
    for epoch in range(args.startEpoch, args.startEpoch + args.epochs):
        ...
        if epoch >= args.generateLabelEpoch:
            ...
            criterion['IntraBCELoss'], criterion['InterBCELoss'] = BCELoss(margin=args.intraBCEMargin, reduce=True, size_average=True).to(device), \
                                                                   BCELoss(margin=args.interBCEMargin, reduce=True, size_average=True).to(device)

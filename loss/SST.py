...
def getIntraPseudoLabel(intraCoOccurrence, target, margin=0.50, output=None):
    """
    ...
    if output != None:
        ...
        output_[output_ < 0.9] = 0
        target_ += output_

    target_ = torch.clamp(target_, min=0, max=1)
...
def getInterPseudoLabel(feature, target, posFeature, margin=0.50, posProb=None):
    """
    ...
    if posProb is not None:
        prob = posProb[np.where(target == 0)[1]]
        posDistance = torch.mean(cos(feature1, feature2) * prob, dim=1)
    else:

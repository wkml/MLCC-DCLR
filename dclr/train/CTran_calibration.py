...
from utils.loss import InstanceContrastiveLoss, PrototypeContrastiveLoss
...
def main(cfg: DictConfig):
    ...
    model = CTranModel(cfg.dataset.class_nums)
    ...
    optimizer = torch.optim.Adam(filter(lambda p: p.requires_grad, model.parameters()),lr=cfg.lr)#, weight_decay=0.0004) 
    # step_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer,mode='min',factor=0.1,patience=5)

    if cfg.evaluate:
        Validate(test_loader, aux_model, model, criterion, 0, cfg)
        return
    ...
    for epoch in range(cfg.start_epoch, cfg.start_epoch + cfg.epochs):
        ...
        mAP, ACE, ECE, MCE = Validate(test_loader, aux_model, model, criterion, epoch, cfg)
...
def Train(train_loader, aux_model, ctran_model, criterion, optimizer, writer, epoch, cfg):
    ctran_model.train()
    ...
    for batch_index, (sample_index, input, target, full_label, mask) in enumerate(train_loader):
        ...
        # Forward
        _, aux_feature = aux_model(input)
        outputs, _, _ = ctran_model(input, mask_in)
...
def Validate(val_loader, model, ctran_model, criterion, epoch, cfg):
    ctran_model.eval()
    ...
    for batch_index, (sample_index, input, target, groundTruth, mask) in enumerate(val_loader):
        ...
        loss_ = torch.sum(unk_mask.cuda() * loss_)
        loss.update(loss_.item(), input.size(0))
